...
class product_template( models.Model ):
    ...
    def _get_partner_rate( self ):
        for obj in self:
            partner_rate = 0.0
            total_messages = len( [x.id for x in obj.website_message_ids if x.skill_rate > 0] )
            if total_messages > 0:
                total_rate = sum( [x.skill_rate for x in obj.website_message_ids] )
                getcontext().prec = 3
    ...
    website_message_ids = fields.One2many( 'website.skills', 'partner_id', domain=lambda self: [( 'website_message', '=', True )], string='Website Comments' )
